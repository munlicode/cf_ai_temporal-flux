...
const toolsRequiringConfirmation = TOOLS_REQUIRING_CONFIRMATION;
...
export default function App() {
  ...
  useEffect(() => {
    localStorage.setItem("flux_mobile_view", mobileView);
  }, [mobileView]);
  ...
  return (
    <div className="h-dvh w-full flex flex-col bg-ob-base-200 text-ob-text-primary overflow-hidden">
      {/* Header */}
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <div
          className={`flex-1 flex flex-col md:min-w-[400px] bg-ob-base-100 relative overflow-hidden ${
            mobileView === "chat" ? "flex" : "hidden md:flex"
          }`}
        >
          {/* Header for Chat */}
          <div className="px-4 md:px-6 py-3 md:py-4 border-b border-ob-border flex justify-between items-center bg-ob-base-100/80 backdrop-blur-md sticky top-0 z-10">
          ...
          {/* Messages Area */}
          <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-4 md:space-y-6 pb-24 scroll-smooth">
            ...
            {agentMessages.map((m, index) => {
              ...
              return (
                ...
                >
                  <div
                    className={`flex gap-2 md:gap-3 max-w-[92%] md:max-w-[85%] ${isUser ? "flex-row-reverse" : "flex-row"}`}
                  >
                    {/* Avatar */}
                    <div
                        ...
                        {m.parts?.map((part, i) => {
                          if (part.type === "text") {
                            ...
                            return (
                              <Card
                                key={`${m.id}-${i}`}
                                className={`p-3 md:p-3.5 rounded-2xl shadow-sm border text-sm leading-relaxed ${
                                  isUser
          ...
          {/* Input Area */}
          <div className="p-3 md:p-4 bg-ob-base-100 border-t border-ob-border">
            <form
              onSubmit={(e) => {
                e.preventDefault();
                handleAgentSubmit(e);
              ...
              className="relative flex items-end gap-2 bg-ob-base-200 rounded-2xl p-1.5 md:p-2 border border-ob-border focus-within:ring-2 focus-within:ring-brand-500/20 focus-within:border-brand-500 transition-all"
            >
              <Textarea
                ref={textareaRef}
                ...
                className="flex-1 bg-transparent border-none focus:ring-0 focus-visible:ring-0 focus-visible:ring-offset-0 p-2 min-h-[24px] max-h-[150px] resize-none text-sm placeholder:text-ob-text-secondary/50 shadow-none"
                value={agentInput}
                onChange={(e) => {
